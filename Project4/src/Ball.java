    ...
    void checkAnimalCollision(final List<Animal> animals) {
        ...
        for (Animal animal : animals) {
            ...
            if (circle.getBoundsInParent().intersects(animal.getBounds())) {
                ...
                if (animal.getBounds().getMaxX() < (x - BALL_RADIUS + 1) && vx < 0) {
                    //checks for hitting right side of animals
                    System.out.println("hits on the right");
                    vx *= -1;
                    animal.deactivate();
                } else if (animal.getBounds().getMinX() > (x + BALL_RADIUS - 1) && vx > 0) {
                ...
                if (animal.getBounds().getMaxY() < (y - BALL_RADIUS + 1) && vy < 0) {
                    //checks for hits on bottom side
                    System.out.println("hits on the bottom");
                    vy *= -1;
                    animal.deactivate();
                } else if (animal.getBounds().getMinY() > (y + BALL_RADIUS - 1) && vy > 0) {
